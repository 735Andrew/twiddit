...
import jwt, json, rq, redis, secrets
from flask import current_app, url_for

followers = sa.Table(
    # Вспомогательная таблица, демонстрирующая представление подписчиков в БД
    "followers",
    db.metadata,
...
class User(PaginatedAPIMixin, UserMixin, db.Model):
    """Представление пользователей в базе данных"""

    id: so.Mapped[int] = so.mapped_column(primary_key=True)
    username: so.Mapped[str] = so.mapped_column(sa.String(64), index=True, unique=True)
    email: so.Mapped[str] = so.mapped_column(sa.String(120), index=True, unique=True)
    # Определение столбца, который может быть пустым или обнуляемым
    ...
    def avatar(self, size):
        # Генерация URL-адреса аватара пользователя через хэш его почты.
        #
        # В случае, если у пользователя не будет ранее созданного профиля
        # на сервисе Gravatar, его аватар - набор геометрических фигур.
        digest = md5(self.email.lower().encode("utf-8")).hexdigest()
        return f"https://www.gravatar.com/avatar/{digest}?d=identicon&s={size}"

    def follow(self, user):
        # Функция подписки пользователя self на пользователя user
        if not self.is_following(user):
            self.following.add(user)

    def unfollow(self, user):
        # Функция отписки пользователя self от пользователя user
        if self.is_following(user):
            self.following.remove(user)

    def is_following(self, user):
        # Функция проверки отслеживания пользователя user пользователем self
        query = self.following.select().where(User.id == user.id)
        return db.session.scalar(query) is not None

    def followers_count(self):
        # Функция проверки количества подписчиков пользователя
    ...
    def following_count(self):
        # Функция проверки количества подписок пользователя
        query = sa.select(sa.func.count()).select_from(
    ...
    def following_posts(self):
        # Функция запроса постов пользователей, на которых подписан пользователь self
    ...
    def launch_task(self, name, description, *args, **kwargs):
        rq_job = current_app.task_queue.enqueue(
            f"tasks.{name}", self.id, *args, **kwargs
        )
...
class Post(db.Model):
    """Представление написанных пользователями постов в базе данных"""
...
def load_user(id):
    ...
    Идентификатор извлекается из сеанса пользователя каждый раз,
    когда тот переходит на новую страницу.
    """
