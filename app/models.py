    ...
    def to_collection_dict(query, page, per_page, endpoint, **kwargs):
        ...
        data = {
            ...
            "_links": {
                ...
                "next": url_for(endpoint, page=page + 1, per_page=per_page, **kwargs) if \
                    resources.has_next else None,
                "prev": url_for(endpoint, page=page - 1, per_page=per_page, **kwargs) if \
                    resources.has_prev else None
            }
        }
        return data
...
class User(PaginatedAPIMixin, UserMixin, db.Model):
    ...
    def launch_task(self, name, description, *args, **kwargs):
        rq_job = current_app.task_queue.enqueue(f'tasks.{name}', self.id, *args, **kwargs)
    ...
    def check_token(token):
        ...
        if user is None or user.token_expiration.replace(tzinfo=timezone.utc) < \
